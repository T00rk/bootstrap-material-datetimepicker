(function($, moment)
{
	...
	Plugin.prototype =
	{
		...
		initTemplate: function()
		{
		    ...
		    this.template = '<div class="dtp hidden" id="' + this.name + '">' +
								'<div class="dtp-content">' +
		                            ...
		                            '</div>'+
                                   '<div class="dtp-year-rank p80"></div>'+
                                     '<div class="right center p10"><a href="javascript:void(0);" class="dtp-select-pick-year-after">'+
                                   ...
                                   '</div>' +
									...
									'<div class="dtp-buttons">' +
										...
										'<button class="dtp-btn-cancel btn ink-reaction btn-raised btn-xs  btn-danger">' + this.params.cancelText + '</button>' +
										'<button class="dtp-btn-ok btn ink-reaction btn-raised btn-xs  btn-primary">' + this.params.okText + '</button>' +
										'<div class="clearfix"></div>' +
									'</div>' +
								'</div>' +
							'</div>';
		...
		initButtons: function()
		{
			...
			this._attachEvent(this.$dtpElement.find('div.dtp-actual-month select'), 'change', this._onActualMonthChange.bind(this));
		...
		initHours: function()
		{
			...
			for(var i = 0; i < 12; i++)
			{
				...
				var fill = ((this.currentDate.format(hFormat) == i) ? "#618A87" : 'transparent');
			...
			if(!this.params.shortTime)
			{
				for(var i = 0; i < 12; i++)
				{
					...
					var fill = ((this.currentDate.format(hFormat) == (i + 12)) ? "#618A87" : 'transparent');
		...
		initMinutes: function()
		{
			...
			for(var i = 0; i < 60; i++)
			{
				...
				var color = ((this.currentDate.format("m") == i) ? "#618A87" : 'transparent');
		...
		createSVGClock : function(isHour)
		{
			...
			if(isHour)
			{
				...
				var svgHourHand = this.createSVGElement("line", { class: 'hour-hand', x1: 0, y1: 0, x2: 0, y2: hl, stroke: '#618A87', 'stroke-width': 8 });
			...
			{
			    var svgMinuteHand = this.createSVGElement("line", { class: 'minute-hand', x1: 0, y1: 0, x2: 0, y2: -150, stroke: '#618A87', 'stroke-width': 2 });
		...
		showDate: function(date)
		{
			if(date)
			{
			    ...
			    if (this.params.changeMonth == false) {
			        this.$dtpElement.find('.dtp-actual-month').html(date.locale(this.params.lang).format('MMM').toUpperCase());
			    } else {
			       
			        this.$dtpElement.find('.dtp-actual-month select').val(date.format('M'));
			    }
		...
		_onActualMonthChange: function () {

		    var month = $("div.dtp-actual-month select").val();
		    var date = new Date(this.currentDate.format('YYYY'), month-1, this.currentDate.format('DD'));
	    ...
	    _onActualYearClick: function() {
	        ...
	        if(this.currentView == 0)
	        {
	            ...
	            $(".dtp-select-year-after").addClass("hidden");
	        }
	        ...
	        var nextYear = parseInt(year) + 23;
	        var yearPicker = "";

	        for (i = year; i <= nextYear; i++)
	        {
	            yearDiv = '<div class="dtp-year-picker-item" data-year="' + i + '">' + i + '</div>';
		...
		_onYearPickBeforeClick: function () {

		    var year = $(".dtp-year-rank").html().substring(0, 4);
            var y = year.substring(3, 4);
            
            year = parseInt(year) - 23;
		    var nextYear = parseInt(year) + 23;
		...
		_onYearPickItenClick: function (e)
		{
		    var s = event.target;
		...
		_onSelectHour: function(e)
		{
			if(!$(e.target).hasClass('disabled'))
			{
				...
				$(parent.find('#h-' + value)).attr('fill', '#618A87');
		...
		_onSelectMinute: function(e)
		{
			if(!$(e.target).hasClass('disabled'))
			{
				...
				$(parent.find('#m-' + value)).attr('fill', '#618A87');
